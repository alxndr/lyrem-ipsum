defmodule LyricsWiki do
  ...
  * monitor & space out requests
  ...
  @regex_chorus ~r/^\[?Chorus:?\]?$/
  @regex_junk_around_match ~r/[\s"\(]*(?<content>.+)[\s;,.\)]*$/
  @regex_number_of_times_in_parens ~r/^\(\d+ times\)$/
  @regex_musician_name_with_instrument_in_parens ~r/^([A-Z][a-zA-Z'-] ){2,}\(.*\)$/
  ...
  defp sanitize_lyrics(lyrics) do
    lyrics
    |> Enum.reduce([], &reducer_sanitize_lyrics/2)
    |> Enum.reverse
  end
  ...
  """
  @spec reducer_sanitize_lyrics(String.t, [String.t]) :: [String.t]
  defp reducer_sanitize_lyrics("", sanitized_lyrics), do: sanitized_lyrics
  defp reducer_sanitize_lyrics(lyric, sanitized_lyrics) do
